...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $entered_otp = implode('', array_map('trim', $otp_digits));
    ...
    if (!preg_match('/^\d{6}$/', $entered_otp)) {
        ...
        // Check in database
        $stmt = $conn->prepare("SELECT * FROM farmers WHERE email = ? AND otp = ?");
        $stmt->execute([$email, $entered_otp]);

        if ($stmt->rowCount() > 0) {
            // Update farmer account
            $update = $conn->prepare("UPDATE farmers SET otp = NULL, certification_status = 'verified' WHERE email = ?");
...
<div class="verify-container">
    ...
    <?php if (!empty($message)): ?>
        <p class="message <?php echo (strpos($message, 'Verified') !== false) ? 'success' : ''; ?>">
            <?php echo $message; ?>
        </p>
    <?php endif; ?>
    ...
    <form method="post" onsubmit="return validateOTP();">
        ...
        <center><label class="form-label">Enter 6-digit OTP sent to your email</label></center>
        <div class="otp-box">
            <?php for ($i = 0; $i < 6; $i++): ?>
                <input type="text" name="otp[]" maxlength="1" class="otp-input" required oninput="moveToNext(this, <?php echo $i; ?>)">
            <?php endfor; ?>
        </div>
